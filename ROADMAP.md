...
| :--- | :--- | :--- | :--- |
| **Password Reset & Email Delivery** (`email_service.py`) | Green card: *"Check your inbox"*. | DigitalOcean droplets block outbound raw SMTP ports (25, 465, 587) by default. The API strictly never returns tokens in HTTP response (prevents account takeover). | **Backlog**: Integrate HTTP-based transactional email API (**Resend / SendGrid / Brevo** via HTTPS Port 443) for 100% guaranteed delivery to corporate inboxes without SMTP port blocks. |
| **User Onboarding / Sign Up Model** (`LoginForm.tsx` / `register`) | "Don't have an account? Sign up". | Direct self-registration creates active users immediately. For strict enterprise multi-tenancy, an admin approval gate is desirable. | **Backlog**: Add `ALLOW_PUBLIC_SIGNUP` config toggle. When false, new users enter `PENDING_APPROVAL` and await admin confirmation before gaining workspace access. |
| **Clipboard Copying on HTTP / IP** (`ChatMessage.tsx`, `files/page.tsx`) | "Copy Message" / "Copied!" button. | `navigator.clipboard` is restricted by modern browsers to Secure Contexts (`https://` or `localhost`). On raw HTTP IP deployments, it throws TypeError without fallback. | **Design Invariant (Fixed)**: Implemented universal fallback via hidden DOM `document.execCommand('copy')` ensuring 100% reliable copy on any protocol. |
