        ...
        if (deletingId === s.id) {
          return (
            <SidebarMenuItem key={s.id} className="group/session-item relative">
              <div className="flex items-center justify-between w-full px-3 py-2 text-xs text-red-400 bg-destructive/10 border border-destructive/30 rounded-md">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <button
                    ...
                    className="p-1 hover:bg-accent text-muted-foreground rounded transition-colors cursor-pointer border-0 bg-transparent"
                    title="Cancel"
                  >
        ...
        return (
          <SidebarMenuItem key={s.id} className="group/session-item relative">
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-muted-foreground hover:text-red-400 hover:bg-red-500/10 rounded-md transition-all duration-200 opacity-0 group-hover/session-item:opacity-100 focus:opacity-100 group-data-[collapsible=icon]:hidden cursor-pointer border-0 bg-transparent"
              title="Delete chat"
            >
...
function UserCard() {
  ...
  return (
    <>
      <div className="group flex items-center gap-3 px-2 py-2 rounded-xl border border-border bg-card/40 hover:bg-card/70 transition-all duration-200 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:p-0 group-data-[collapsible=icon]:border-transparent group-data-[collapsible=icon]:bg-transparent group-data-[collapsible=icon]:w-full">
        ...
        >
          <span className="text-[11px] text-muted-foreground font-medium">Signed in as</span>
          <span className="text-[12px] text-foreground/80 font-semibold truncate hover:text-primary transition-colors">
            {email ?? '...'}
          </span>
        </div>
        ...
        <button
          ...
          title="Sign out"
          className="group-data-[collapsible=icon]:hidden shrink-0 p-1.5 rounded-md text-muted-foreground hover:text-red-400 hover:bg-red-500/10 transition-all duration-200 disabled:opacity-50 border-0 bg-transparent cursor-pointer"
        >
...
function SpaceSwitcher() {
  ...
  return (
    <div className="px-2 mb-4 group-data-[collapsible=icon]:hidden relative">
      <div className="flex items-center justify-between text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mb-2 px-1">
        ...
        {currentSpace && (
          <div className="flex items-center gap-1">
            <button
              ...
              className="p-1 hover:bg-accent text-muted-foreground hover:text-primary rounded transition-colors cursor-pointer border-0 bg-transparent"
      ...
      <button
        ...
        className="w-full flex items-center justify-between px-3 py-2.5 text-xs text-foreground/80 border border-border bg-card/60 hover:bg-card rounded-lg shadow-sm transition-all duration-200 cursor-pointer select-none text-left"
      >
        <span className="truncate font-semibold text-foreground">
          {currentSpace ? currentSpace.name : "Global Workspace"}
        </span>
        <ChevronDown className="w-3.5 h-3.5 text-muted-foreground shrink-0 ml-1 transition-transform" />
      </button>
      ...
      {isOpen && (
        <div className="absolute left-2 right-2 mt-1 bg-card border border-border rounded-xl shadow-xl z-50 p-1.5 animate-in fade-in slide-in-from-top-1 duration-100">
          <div className="max-h-40 overflow-y-auto mb-1.5 scrollbar-thin">
            <button
              ...
              className={`w-full text-left px-3 py-2 text-xs rounded-md transition-colors cursor-pointer border-0 bg-transparent ${
                !currentSpaceId ? "bg-accent text-foreground font-bold" : "text-muted-foreground hover:bg-accent/50 hover:text-foreground"
              }`}
            >
              {t.nav.allSpaces}
            </button>
            {spaces.map(s => (
              <div
                ...
                className={`group/space flex items-center justify-between rounded-md transition-colors ${
                  s.id === currentSpaceId ? "bg-accent" : "hover:bg-accent/50"
                }`}
              >
                <button
                  ...
                  className={`flex-1 text-left px-3 py-2 text-xs rounded-md transition-colors truncate cursor-pointer border-0 bg-transparent ${
                    s.id === currentSpaceId ? "text-foreground font-bold" : "text-muted-foreground hover:text-foreground"
                  }`}
                >
                  {s.name}
                </button>
                <div className="flex items-center gap-1 opacity-0 group-hover/space:opacity-100 transition-opacity mr-1.5 shrink-0">
                  <button
                    ...
                    className="p-1 text-muted-foreground hover:text-primary hover:bg-accent rounded transition-colors cursor-pointer bg-transparent border-0"
                  ...
                  <button
                    ...
                    className="p-1 text-muted-foreground hover:text-red-400 hover:bg-red-500/10 rounded transition-colors cursor-pointer bg-transparent border-0"
          ...
          <div className="border-t border-border pt-1.5 mt-1.5 px-1.5">
            {isCreating ? (
              <form onSubmit={handleCreateSpace} className="flex flex-col gap-1.5 animate-in zoom-in-95 duration-150">
                ...
                <textarea
                  ...
                  className="w-full bg-background border border-border rounded px-2 py-1 text-xs text-foreground focus:outline-none focus:border-primary resize-none h-12"
                />
                <div className="flex gap-1 justify-end">
                  <button
                    type="button"
                    onClick={() => setIsCreating(false)}
                    className="px-2 py-1 text-[10px] text-muted-foreground hover:bg-accent rounded cursor-pointer border-0 bg-transparent"
                  >
                    {t.common.cancel}
                  </button>
                  <button
  ...
  return (
    <>
      <Sidebar collapsible="icon" className="border-r border-border bg-sidebar z-40">
        <SidebarHeader className="h-16 flex items-center justify-center p-2 border-b border-border group-data-[collapsible=icon]:p-0">
          <button 
          ...
          >
            <div className="relative flex h-8 w-8 shrink-0 items-center justify-center transition-all duration-200 hover:scale-105 active:scale-95">
              {hovered ? (
                ...
                // eslint-disable-next-line @next/next/no-img-element
                <img src="/logo-icon.png" alt="Neurach" className="h-6 w-6 object-contain drop-shadow-[0_0_6px_rgba(129,140,248,0.2)]" />
              )}
            </div>
            ...
            <div className="flex items-center group-data-[collapsible=icon]:hidden transition-opacity duration-200">
              <span className="text-[16px] font-bold tracking-tight text-foreground font-sans">
                Neurach
              </span>
            </div>
          </button>
        ...
        <SidebarContent className="p-2 pt-4 group-data-[collapsible=icon]:p-0 group-data-[collapsible=icon]:pt-2">
          ...
          <SidebarGroup className="group-data-[collapsible=icon]:px-0">
            <SidebarGroupLabel className="mb-2 px-2 text-[11px] font-semibold uppercase tracking-widest text-muted-foreground group-data-[collapsible=icon]:hidden">
              {t.nav.platformOps}
            </SidebarGroupLabel>
            <SidebarMenu>
              ...
              <SidebarMenuItem>
                <SidebarMenuButton
                  ...
                  className="text-muted-foreground hover:bg-primary/10 hover:text-primary rounded-md transition-all duration-200 cursor-pointer"
                >
          ...
          <SidebarGroup className="mt-4 group-data-[collapsible=icon]:hidden group-data-[collapsible=icon]:px-0">
            <SidebarGroupLabel className="mb-2 px-2 text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">
              {t.nav.recentChats}
            </SidebarGroupLabel>
            <SidebarMenu>
              <SidebarMenuItem>
        ...
        <SidebarFooter className="border-t border-border p-2 group-data-[collapsible=icon]:p-0 group-data-[collapsible=icon]:py-2 group-data-[collapsible=icon]:flex group-data-[collapsible=icon]:justify-center">
          <UserCard />
        </SidebarFooter>
      </Sidebar>
