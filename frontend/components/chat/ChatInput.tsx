...
export function ChatInput({ isLoading, isProcessingFiles = false, onSubmit }: ChatInputProps) {
  ...
  return (
    <div className="w-full flex flex-col items-center">
              ...
              <div
                ...
                className={`flex items-center gap-2 border rounded-xl py-1 pl-3 pr-2 text-xs shadow-lg transition-all duration-200 ${
                  isProcessingFiles
                    ...
                    : "bg-card/90 border-border text-foreground/80 backdrop-blur-md"
                }`}
              >
                {isProcessingFiles ? (
                ...
                {isProcessingFiles ? (
                  ...
                  <button
                    ...
                    className="p-0.5 hover:bg-accent rounded text-muted-foreground hover:text-foreground transition-colors border-0 bg-transparent cursor-pointer"
                    disabled={isDisabled}
                  >
                    <X className="w-3 h-3" />
                  </button>
      ...
      <div className={`relative flex items-center w-full bg-card/80 hover:bg-card/90 backdrop-blur-2xl border rounded-[28px] shadow-2xl transition-all duration-200 focus-within:border-primary/40 focus-within:ring-2 focus-within:ring-primary/10 focus-within:bg-card/95 px-2.5 py-1.5 ${
        isProcessingFiles ? "border-amber-500/30" : "border-border"
      }`}>
        {/* Hidden File Input */}
        <input 
          type="file" 
        ...
        <button 
          ...
          className="flex items-center justify-center h-8 w-8 rounded-full text-muted-foreground hover:text-foreground hover:bg-accent/50 transition-all flex-shrink-0 border-0 bg-transparent cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
        ...
        <textarea
          ...
          className="w-full max-h-[180px] min-h-[36px] bg-transparent border-0 resize-none py-2 px-2.5 text-sm sm:text-[14.5px] text-foreground placeholder:text-muted-foreground focus:ring-0 focus:outline-none overflow-y-auto custom-scrollbar leading-relaxed"
          rows={1}
          disabled={isDisabled}
        />
        ...
        <button
          ...
          className={`flex items-center justify-center h-8 w-8 rounded-full flex-shrink-0 transition-all duration-200 border-0 cursor-pointer ${
            hasContent
              ? "bg-primary text-primary-foreground shadow-md shadow-primary/30 hover:opacity-90 active:scale-95"
      ...
      <div className="text-center mt-1.5 text-[10px] text-muted-foreground/60 tracking-wide select-none">
        {isProcessingFiles
